...
function render() {
  ...
  var store = this;
  var mapExists = store.ctx.map && store.ctx.map.getSource(SOURCE) !== undefined;
  ...
  var newIds = store.getAllIds();

  var lastCount = store.source.length;
  store.source = [];

  var changed = lastCount !== store.source.length || newIds.length > 0;
  newIds.forEach(function (id) { return renderFeature(id); });
  ...
  if (changed) {
    store.ctx.map.getSource(SOURCE).setData({
...
Store.prototype.setSelected = function (featureIds, options) {
  ...
  this.deselect(
    this._selectedFeatureIds
      .values()
      .filter(function (id) { return featureIds.indexOf(id) === -1; }),
...
Store.prototype.storeMapConfig = function () {
  ...
  interactions.forEach(function (interaction) {
    ...
    if (interactionSet) {
      this$1$1._mapInitialConfig[interaction] =
        this$1$1.ctx.map[interaction].isEnabled();
    }
  });
};
...
SimpleSelect.fireFeatureDragStart = function (state) {
  ...
  this.map.fire(events$1.FEATURE_DRAG_START, {
    action: updateActions.MOVE,
    features: this.getSelected().map(function (f) { return f.toGeoJSON(); }),
  });
};
...
SimpleSelect.fireFeatureDragging = function () {
  ...
  this.map.fire(events$1.FEATURE_DRAGGING, {
    action: updateActions.MOVE,
    features: this.getSelected().map(function (f) { return f.toGeoJSON(); }),
  });
};
...
SimpleSelect.fireFeatureDragEnd = function (state) {
  ...
  this.map.fire(events$1.FEATURE_DRAG_END, {
    action: updateActions.MOVE,
    features: this.getSelected().map(function (f) { return f.toGeoJSON(); }),
  });
};
...
SimpleSelect.onMouseMove = function (state, e) {
  ...
  if (isFeature$1 && state.dragMoving) { this.fireUpdate(state); }
...
function setupAPI (ctx, api) {
  ...
  api.add = function (geojson) {
    ...
    var ids = featureCollection.features.map(function (feature) {
      ...
      if (
        ...
        if (
          !isEqual(
            internalFeature$1.getCoordinates(),
  ...
  api.delete = function (featureIds) {
    ...
    if (
      ...
      ctx.events.changeMode(modes$1.SIMPLE_SELECT, undefined, {
        silent: true,
      });
    } else {
      ctx.store.render();
  ...
  api.changeMode = function (mode, modeOptions) {
    ...
    if (
      mode === modes$1.SIMPLE_SELECT &&
      api.getMode() === modes$1.SIMPLE_SELECT
    ) {
      if (
        stringSetsAreEqual(
